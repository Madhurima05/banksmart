...
const users = [
  { id: 1, name: 'Madhurima Kuchulakanti', email: 'madhurima@banksmart.com', password: 'Smart@123', phone: '+1 (555) 234-5678', accountNumber: 'BS-4821-9034-7651', balance: 24850.75, savingsBalance: 12400.00, creditScore: 785 }
];
...
app.post('/api/login', (req, res) => {
  ...
  res.json({ success: true, message: 'Login successful', user: { id: user.id, name: user.name, email: user.email, accountNumber: user.accountNumber } });
});
...
app.post('/api/contact', (req, res) => {
  ...
  if (!email.includes('@')) return res.status(400).json({ success: false, message: 'Invalid email format' });
  if (message.length < 10) return res.status(400).json({ success: false, message: 'Message too short' });
  res.json({ success: true, message: 'Message sent successfully' });
});

app.post('/api/transfer', (req, res) => {
  const { fromUserId, toAccount, amount, description } = req.body;
  if (!fromUserId || !toAccount || !amount) return res.status(400).json({ success: false, message: 'All fields are required' });
  if (amount <= 0) return res.status(400).json({ success: false, message: 'Amount must be greater than zero' });
  if (amount > 10000) return res.status(400).json({ success: false, message: 'Transfer limit exceeded' });
  ...
  const newTransaction = { id: transactions.length + 1, userId: user.id, type: 'debit', amount: parseFloat(amount), description: description || 'Transfer to ' + toAccount, date: new Date().toISOString().split('T')[0], category: 'Transfer', status: 'completed' };
...
app.get('/api/dashboard/:userId', (req, res) => {
  ...
  res.json({ success: true, balance: user.balance, savingsBalance: user.savingsBalance, creditScore: user.creditScore, accountNumber: user.accountNumber, recentTransactions: userTransactions.slice(0, 5) });
});
...
app.get('/api/account/:userId', (req, res) => {
  const user = users.find(u => u.id === parseInt(req.params.userId));
  if (!user) return res.status(404).json({ success: false, message: 'User not found' });
  res.json({ success: true, user: { id: user.id, name: user.name, email: user.email, phone: user.phone, accountNumber: user.accountNumber, creditScore: user.creditScore } });
});
